...
// --- Static Concept # formatting ---
// In static mode, users should input ONLY the numeric portion (e.g. 1.01, 21.03, 111.01).
// The generator must always prefix it as: statS{number}
// If someone pastes an old value like stat101 or statS101.01, we normalize safely.
const formatStaticConcept = (raw: unknown): string => {
  const s = clean(raw);
  if (!s) return '';

  // Strip any leading legacy prefixes (stat / statS), then keep digits and dots.
  const stripped = s
    .replace(/^stat\s*s?/i, '')
    .replace(/[^0-9.]/g, '');

  // Keep only the first dot (if any).
...
export const generateStaticOutputs = (data: FormData): GeneratedOutputs => {
  ...
  const adSet = textJoinWith('_', [
    ...
    formatStaticConcept(data.assetId), // E (Concept #) => statS{number}
    data.focus,            // F
    data.theme,            // G
  ...
  const adLevelRaw = textJoin('_', [
    ...
    data.trial,
    data.conceptId,
    formatStaticConcept(data.assetId),
    data.focus,
    data.theme,
    data.shortConceptDesc,
    data.carouselCode,
